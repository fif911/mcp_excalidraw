#!/usr/bin/env python3
"""
Build diagram v51 — AWS Perspective (Data Discovery) Solution Architecture
...
# Single source of truth for all arrow styling — unpack with **ARROW_STYLE
...
WEBUI_X = 90;   WEBUI_Y = 70;   WEBUI_W = 380;  WEBUI_H = 470

COST_X = 850;   COST_Y = 70;    COST_W = 820;   COST_H = 250

CAPI_X = 560;   CAPI_Y = 400;   CAPI_W = 260;   CAPI_H = 200
...
STOR_X = 90;    STOR_Y = 570;   STOR_W = 450;   STOR_H = 150
...
COGNITO_CX = 290;      COGNITO_CY = 275
CLOUDFRONT_CX = 110;   CLOUDFRONT_CY = 440
S3_WEBUI_CX = 290;     S3_WEBUI_CY = 440   # aligned under Cognito
...
# Storage management
AMPLIFY_CX = 180;      AMPLIFY_CY = 650
S3_AMPLIFY_CX = 400;   S3_AMPLIFY_CY = 650
...
icon_label_component("users", "file-users", LBL_USERS,
                      cx=USERS_CX, cy=icy_to_cy(USERS_CY, LBL_USERS),
                      icon_size=SVC_ICON, font_size=FONT_BODY, text_color=DARK)
...
# ===================================================
# Arrows with label_number auto-place a numbered circle at the midpoint.
# For L-shaped arrows or custom positions, circles are created separately below.

# A1: Users → CloudFront [C1]
arrow("a-users-cf", USERS_CX + 25, CLOUDFRONT_CY,
      CLOUDFRONT_CX - ICON_R, CLOUDFRONT_CY,
      **ARROW_STYLE, label_number=1)

# A2: CloudFront → S3 WebUIBucket [C2]
arrow("a-cf-s3webui", CLOUDFRONT_CX + ICON_R, CLOUDFRONT_CY,
      S3_WEBUI_CX - ICON_R, S3_WEBUI_CY,
      **ARROW_STYLE, label_number=2)

# A3: S3 WebUIBucket → Cognito (straight vertical — same X axis)
# Arrow stops below Cognito label with 7px visual gap
COGNITO_LABEL_BOTTOM = COGNITO_CY + ICON_R + COMP_GAP + FONT_BODY * 1.25
arrow("a-s3webui-cognito", S3_WEBUI_CX, S3_WEBUI_CY - ICON_R,
      COGNITO_CX, COGNITO_LABEL_BOTTOM + 7,
      **ARROW_STYLE)

# A4: Lambda Settings → Client API vertical trunk [C5 — ORANGE]
arrow("a-lambda-set-capi", LAMBDA_SET_CX, LAMBDA_SET_CY + LABEL_2LINE_H,
      LAMBDA_SET_CX, CAPI_Y,
      **ARROW_STYLE, label_number=5, label_bg=ORANGE)
...
# A5: Lambda Settings → DynamoDB [C6 — ORANGE, override position outside Web UI border]
arrow("a-lambda-dynamo", LAMBDA_SET_CX - ICON_R, DYNAMO_CY,
      DYNAMO_CX + ICON_R, DYNAMO_CY,
      **ARROW_STYLE, label_number=6, label_bg=ORANGE,
      label_cx=510, label_cy=DYNAMO_CY - COFFSET)  # override: keep circle outside Web UI right border

# A6: Client API ↔ VPC (bidirectional)
CAPI_MID_Y = CAPI_Y + CAPI_H / 2
arrow("a-capi-vpc", CAPI_X + CAPI_W, CAPI_MID_Y,
      VPC_X, CAPI_MID_Y,
      **ARROW_STYLE, start_arrowhead="arrow", end_arrowhead="arrow")
...
# A8: Fargate → API GW Gremlin (L-shape, exits VPC left) [C17 on last segment]
EXIT_X = VPC_X - 15
arrow("a-fargate-grem", FARGATE_CX - ICON_R, FARGATE_CY,
      ...
      **ARROW_STYLE,
      ...
      label_number=17, label_segment=-1)

# A9: Lambda Gremlin ↔ Neptune (bidirectional) [C7]
...
# A10: Lambda Search ↔ OpenSearch (bidirectional) [C8]
...
# A13b: Lambda Cost → S3 AthenaResultsBucket (skip arrow, U-shape below row) [C12]
SKIP_Y = 295
...
# A14: Amplify ↔ S3 AmplifyStorageBucket (bidirectional) [C3]
...
# A15: Container image → ECR [C14 — override: midpoint lands on VPC border, shift right]
...
arrow("a-imgd-ecr", CI_CX - ICON_R, CI_ICY,
      ...
      **ARROW_STYLE, label_number=14,
      label_cx=VPC_X + VPC_W + 40, label_cy=ECR_CY - COFFSET)

# A15b: ECS → ECR (L-shape: right from ECS, then down to ECR)
arrow("a-ecs-ecr", ECS_CX + ICON_R, ECS_CY,
      ECR_CX, ECR_CY - ICON_R,
      **ARROW_STYLE,
      waypoints=[(ECR_CX, ECS_CY)])

# A16: ECS → Fargate (L-shape) [C15 on first segment (horizontal)]
arrow("a-ecs-fargate", ECS_CX - ICON_R, ECS_CY,
      FARGATE_CX, FARGATE_CY - ICON_R,
      **ARROW_STYLE,
      waypoints=[(FARGATE_CX, ECS_CY)],
      label_number=15, label_segment=0)

# A17: Fargate → SDK (L-shape, exits VPC left) [C16 on last segment]
# Stagger vertical lines with equal 20px gaps: Gremlin=EXIT_X, SDK=EXIT_X-20, Config=EXIT_X-40
# Keeps all last horizontal segments > 50px so arrowheads render consistently
arrow("a-fargate-sdk", FARGATE_CX - ICON_R, FARGATE_CY,
      SDK_CX + ICON_R, SDK_CY,
      **ARROW_STYLE,
      waypoints=[(EXIT_X - 20, FARGATE_CY), (EXIT_X - 20, SDK_CY)],
      label_number=16, label_segment=-1)

# A18: Fargate → Config (L-shape, exits VPC left)
arrow("a-fargate-config", FARGATE_CX - ICON_R, FARGATE_CY,
      CONFIG_CX + ICON_R, CONFIG_CY,
      **ARROW_STYLE,
      waypoints=[(EXIT_X - 40, FARGATE_CY), (EXIT_X - 40, CONFIG_CY)])

time.sleep(0.3)

# ===================================================
# 13. NUMBERED CIRCLES — standalone (no associated arrow)
# ===================================================
# All arrow-attached circles are now embedded via label_number/label_segment above.
# Only circles with no direct arrow remain here.

# Circle 4 — between API Gateway and AppSync (GREEN, no associated arrow)
numbered_circle("c4", 4, cx=(APIGW_CX + APPSYNC_CX) / 2, cy=APIGW_CY, size=CIRCLE_SIZE, bg_color=GREEN_C4, font_size=FONT_BODY)

# Circle 13 — right of CodePipeline→CodeBuild arrow (vertical_stack arrow, not accessible)
