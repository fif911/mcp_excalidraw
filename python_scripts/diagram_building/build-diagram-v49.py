#!/usr/bin/env python3
"""
Build diagram v49 — AWS Perspective (Data Discovery) Solution Architecture
...
register_icon_pack("v49", {
    ...
    # S3 — use Architecture icon (green filled square), not Resource icon (outline only)
...
SVC_ICON = 65  # reference uses ~64-70px service icons
HDR_ICON = 40
FONT_HDR = 22
FONT_BODY = 16
...
def icy_to_cy(target_icon_cy, label_text, icon_size=SVC_ICON, gap=COMP_GAP, font_size=FONT_BODY):
    """Compute the component center cy so the icon image center lands at target_icon_cy.
    This ensures connected icons sharing the same target Y are perfectly aligned."""
...
# ===================================================
# CONTAINER COORDINATES — derived from content, tight padding
# ===================================================
CLOUD_X = 65;   CLOUD_Y = 20;   CLOUD_W = 1900; CLOUD_H = 1250

WEBUI_X = 90;   WEBUI_Y = 70;   WEBUI_W = 480;  WEBUI_H = 410

# Cost: derived from icon span, tall enough to contain skip arrow + circle 12
# Skip arrow at y=295, circle 12 at y=272, circle bottom=290 → need bottom > 305
COST_X = 850;   COST_Y = 70;    COST_W = 820;   COST_H = 250

# Client API — will be auto-sized by fit_container
CAPI_X = 560;   CAPI_Y = 400;   CAPI_W = 240;   CAPI_H = 200

# VPC pushed down 50px from cost bottom (320) to avoid overlap
...
# ===================================================
# SERVICE POSITIONS (cx, cy) — centered in their containers
# ===================================================
# Web UI (unchanged)
...
# Lambda Settings (standalone, right of Web UI, same Y as DynamoDB)
LAMBDA_SET_CX = 640;   LAMBDA_SET_CY = 155

# Cost row — icons derived from COST container (center = COST_X + COST_W/2 = 1260)
COST_ROW_Y = 150
COST_CTR = COST_X + COST_W // 2  # 1260
LAMBDA_COST_CX = COST_CTR - 300;  LAMBDA_COST_CY = COST_ROW_Y  # 960
ATHENA_CX = COST_CTR - 150;       ATHENA_CY = COST_ROW_Y       # 1110
S3_CUR_CX = COST_CTR;             S3_CUR_CY = COST_ROW_Y       # 1260
CUR_CX = COST_CTR + 150;          CUR_CY = COST_ROW_Y          # 1410
S3_ATHENA_CX = COST_CTR + 300;    S3_ATHENA_CY = COST_ROW_Y    # 1560

# Client API (pushed down with VPC shift)
APIGW_CX = 620;        APIGW_CY = 505
APPSYNC_CX = 740;      APPSYNC_CY = 505

# Data component (centered: DATA_X=920, DATA_W=560 → center=1200)
# Spread icons 260px apart horizontally, 160px vertically to avoid label overlap
DATA_CTR = DATA_X + DATA_W // 2  # 1200
...
# Discovery component — hierarchical layout per reference:
#   ECS top-center, Fargate bottom-left, ECR bottom-right
DISC_CTR = DISC_X + DISC_W // 2  # 1200
ECS_CX = DISC_CTR;               ECS_CY = 950
FARGATE_CX = DISC_CTR - 150;     FARGATE_CY = 1050
ECR_CX = DISC_CTR + 150;         ECR_CY = 1050

# Image deployment (will use vertical_stack — this is the column center X)
IMGD_CX = 1690
...
# Standalone services — spread for SVC_ICON=65 clearance
APIGW_GREM_CX = 700;   APIGW_GREM_CY = 670
SDK_CX = 700;           SDK_CY = 800
CONFIG_CX = 700;        CONFIG_CY = 920

# External — 15px gap to AWS Cloud left border (CLOUD_X=65, icon right=40)
USERS_CX = 15;          USERS_CY = 410

# Arrow text-side offset: distance from icon center to label bottom + gap (for 2-line text)
# With icy_to_cy: icon center = target CY, label bottom = CY + icon_size/2 + gap + text_h + margin
...
# ===================================================
# icy_to_cy ensures the ICON IMAGE CENTER lands at the base CY value.
# Connected icons sharing the same CY → perfectly aligned arrow Y.
...
# ===================================================
IMGD_START_Y = IMGD_Y + 130  # tuned so Container image icon_cy = ECR_CY = 1050
imgd_stack = vertical_stack(
    ...
    spacing=55,  # generous gap between components for visible arrows
...
# ===================================================
# 12. ARROWS — icon-to-icon connections (cross borders freely)
# ===================================================
# RULE: All arrows use stroke_width=2 consistently.
# RULE: Endpoints touch icon edges (icon_cx ± ICON_R), no gaps, no overlap.
# RULE: Verify uni- vs bidirectional per reference diagram.

# HARD RULE: Horizontal arrows — X at icon edge (cx ± ICON_R), Y centered on icon (cy).
# Vertical arrows — Y at icon edge (cy ± ICON_R), X centered on icon (cx).

# Now base CY values ARE the actual icon image center Y (thanks to icy_to_cy).
# Horizontal arrows: X at icon edge, Y at shared base CY.
...
# A4: Lambda Settings → Client API vertical line
...
# A5: Lambda Settings → DynamoDB (same CY = DYNAMO_CY = LAMBDA_SET_CY = 155)
arrow("a-lambda-dynamo", LAMBDA_SET_CX - ICON_R, DYNAMO_CY,
      DYNAMO_CX + ICON_R, DYNAMO_CY,
      stroke_color=DARK, stroke_width=2)

# A6: Client API ↔ VPC (container-to-container, no icon centers)
...
# A14: Amplify ↔ S3 AmplifyStorageBucket (same CY = 590)
arrow("a-amplify-s3", AMPLIFY_CX + ICON_R, AMPLIFY_CY,
      S3_AMPLIFY_CX - ICON_R, S3_AMPLIFY_CY,
      stroke_color=DARK, stroke_width=2,
      start_arrowhead="arrow", end_arrowhead="arrow")

# A15: Container image → ECR (horizontal, both at ECR_CY=1020)
...
# A15b: ECR → ECS (L-shape: up from ECR top, then left to ECS right edge)
arrow("a-ecr-ecs", ECR_CX, ECR_CY - ICON_R,
      ECS_CX + ICON_R, ECS_CY,
      stroke_color=DARK, stroke_width=2,
      waypoints=[(ECR_CX, ECS_CY)])

# A16: ECS → Fargate (L-shape: left from ECS left edge, then down to Fargate top)
...
# 13. NUMBERED CIRCLES
# ===================================================
# HARD RULE: Circles must NEVER touch or overlap arrows.
# Always offset by COFFSET above/below the arrow line.
# Every circle MUST be adjacent to a visible arrow.
...
# Circle 4 — between API Gateway and AppSync icons, at icon center level
numbered_circle("c4", 4, cx=(APIGW_CX + APPSYNC_CX) / 2, cy=APIGW_CY, size=CIRCLE_SIZE, bg_color="#2f9e44", font_size=FONT_BODY)

# Circle 5 — right of Lambda Settings→Client API vertical arrow
C5_CY = (LAMBDA_SET_CY + LABEL_2LINE_H + CAPI_Y) / 2  # midpoint of arrow
numbered_circle("c5", 5, cx=LAMBDA_SET_CX + COFFSET, cy=C5_CY, size=CIRCLE_SIZE, bg_color="#e8590c", font_size=FONT_BODY)

# Circle 6 — above Lambda Settings→DynamoDB arrow
C6_CX = (DYNAMO_CX + LAMBDA_SET_CX) / 2
numbered_circle("c6", 6, cx=C6_CX, cy=DYNAMO_CY - COFFSET, size=CIRCLE_SIZE, bg_color="#e8590c", font_size=FONT_BODY)
...
# Circle 11 — above CUR→CURBucket arrow
...
# Circle 12 — above skip arrow horizontal segment (between cost and VPC)
...
# Circle 13 — right of arrow between CodePipeline and CodeBuild
...
# Circle 14 — above IMGD→ECR arrow (at ECR_CY), outside VPC
...
# Circle 15 — offset from ECS→Fargate diagonal arrow midpoint
...
# Circle 16 — above A17 horizontal segment (SDK to EXIT_X-40 at y=SDK_CY)
...
# Circle 17 — above A8 horizontal segment (APIGW_GREM to EXIT_X at y=APIGW_GREM_CY)
