#!/usr/bin/env python3
"""
Build diagram v46 — Data Transfer Hub Architecture
...
register_icon_pack("v46", {
    ...
    "file-dynamodb":      "Architecture-Service-Icons_01302026/Arch_Databases/48/Arch_Amazon-DynamoDB_48.svg",
    "file-appsync":       "Architecture-Service-Icons_01302026/Arch_Application-Integration/48/Arch_AWS-AppSync_48.svg",
    "file-lambda":        "Architecture-Service-Icons_01302026/Arch_Compute/48/Arch_AWS-Lambda_48.svg",
    "file-cloudformation":"Architecture-Service-Icons_01302026/Arch_Management-Tools/48/Arch_AWS-CloudFormation_48.svg",
    "file-cloudfront":    "Architecture-Service-Icons_01302026/Arch_Networking-Content-Delivery/48/Arch_Amazon-CloudFront_48.svg",
    "file-s3":            "Architecture-Service-Icons_01302026/Arch_Storage/48/Arch_Amazon-Simple-Storage-Service_48.svg",
    "file-fargate":       "Architecture-Service-Icons_01302026/Arch_Containers/48/Arch_AWS-Fargate_48.svg",
    # Services — Managed Account
    "file-ecr":           "Architecture-Service-Icons_01302026/Arch_Containers/48/Arch_Amazon-Elastic-Container-Registry_48.svg",
    # Resource icons
    "file-user":          "Resource-Icons_01302026/Res_General-Icons/Res_48_Light/Res_User_48_Light.svg",
    "file-client":        "Resource-Icons_01302026/Res_General-Icons/Res_48_Light/Res_Client_48_Light.svg",
    "file-cfn-template":  "Resource-Icons_01302026/Res_Management-Governance/Res_AWS-CloudFormation_Template_48.svg",
...
# Customer's AWS Account (extended right to give room for circles in gap)
...
# AWS Step Functions workflow (pushed right to fit circle 5 between Lambda and SF)
SF_X = 670;     SF_Y = 175;     SF_W = 230;      SF_H = 365
# SF right = 900, SF bottom = 540

# AWS Managed Account (pushed right for circle clearance)
...
# DynamoDB (shifted left to clear SF border)
...
# Inside Step Functions (centered in new SF_X=670, SF_W=230)
LAMBDA_SF_CX = 785;     LAMBDA_SF_CY = 290
CLOUDFORM_CX = 785;     CLOUDFORM_CY = 440

# Bottom row (customer account)
CLOUDFRONT_CX = 380;    CLOUDFRONT_CY = 640
S3_CUST_CX = 570;       S3_CUST_CY = 640
FARGATE_CX = 785;       FARGATE_CY = 640
...
# --- Managed Account service positions (shifted +50 with MA_X) ---
S3_TPL_CX = 1110;       S3_TPL_CY = 250
DDB_TPL_CX = 1330;      DDB_TPL_CY = 250

S3_MA_CX = 1110;        S3_MA_CY = 440
ECR_TPL_CX = 1330;      ECR_TPL_CY = 440

ECR_CX = 1110;          ECR_CY = 640
DOCKER_CX = 1330;       DOCKER_CY = 640

# --- Circle positions (offset from arrow paths with 5px margin) ---
# Horizontal arrows: circle ABOVE → CY = arrow_y - CIRCLE_R - 5 - stroke/2 = arrow_y - 26
# Vertical arrows:   circle LEFT  → CX = arrow_x - CIRCLE_R - 5 - stroke/2 = arrow_x - 26
COFFSET = CIRCLE_R + 5 + 1   # 26: radius + 5px gap + half stroke width

BEND_CF = 290                 # x for DTH→CloudFront L-shape bend (circle 1 sits left of this)

C1_CX = BEND_CF - COFFSET;       C1_CY = 590     # left of vertical segment at x=290
C2_CX = 290;                     C2_CY = APPSYNC_CY - COFFSET  # above horizontal at y=390
C3_CX = DTH_CX - COFFSET;        C3_CY = 450     # left of vertical trunk at x=120
C4_CX = 480;                     C4_CY = APPSYNC_CY - COFFSET  # above horizontal at y=390
C5_CX = 631;                     C5_CY = APPSYNC_CY - COFFSET  # above horizontal at y=390
GAP_CX = (CUST_X + CUST_W + MA_X) / 2          # center of gap between accounts
C6_CX = GAP_CX;                     C6_CY = S3_MA_CY    # in gap, on S3←CFn arrow
C7_CX = GAP_CX;                     C7_CY = ECR_CY      # in gap, on ECR→Fargate arrow
C8_CX = 435;                     C8_CY = DYNAMO_CY - COFFSET    # above horizontal, left of arrowhead
...
icon_label_component("openid", "file-openid", "OpenID\nConnect",
...
icon_label_component("s3-tpl", "file-cfn-template", "S3 replication\ncomponent template",
                      cx=S3_TPL_CX, cy=S3_TPL_CY + ICON_VSHIFT,
                      icon_size=SVC_ICON, font_size=FONT_BODY, text_color=DARK)

icon_label_component("ddb-tpl", "file-cfn-template", "DynamoDB\nreplication\ncomponent template",
...
icon_label_component("ecr-tpl", "file-cfn-template", "ECR replication\ncomponent\ntemplate",
...
# ===================================================
# 6. ARROWS — full continuous lines; circles sit ON TOP (same dark color hides overlap)
# ===================================================
...
# --- DTH vertical trunk (circle 3 sits on it, arrow hidden under dark circle) ---
...
# --- DTH → CloudFront (L-shape right-down-right, circle 1 sits left of vertical segment) ---
...
# --- AppSync → DynamoDB (routed right of Auth, circle 8 sits on horizontal segment) ---
...
# --- CloudFormation → Fargate (from SF bottom edge straight down) ---
SF_BOTTOM = SF_Y + SF_H
arrow("a-cfn-fargate", CLOUDFORM_CX, SF_BOTTOM,
      FARGATE_CX, FARGATE_CY - ICON_R,
      stroke_color=DARK, stroke_width=2)

# --- S3 managed → CloudFormation (arrow points left, circle 6 in gap) ---
arrow("a-s3ma-cfn", S3_MA_CX - ICON_R, S3_MA_CY,
      SF_X + SF_W, CLOUDFORM_CY,
      stroke_color=DARK, stroke_width=2)

# --- ECR → Fargate (arrow points left toward Fargate, circle 7 sits on it) ---
arrow("a-ecr-fargate", ECR_CX - ICON_R, ECR_CY,
      FARGATE_CX + ICON_R, FARGATE_CY,
      stroke_color=DARK, stroke_width=2)

# --- No internal arrows in Managed Account (templates are standalone) ---

time.sleep(0.3)

# ===================================================
# 7. NUMBERED CIRCLES (AFTER arrows — higher z-order covers arrow lines)
# ===================================================
