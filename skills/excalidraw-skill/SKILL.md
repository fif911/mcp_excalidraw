...
> 6. Or add as MCP server: `claude mcp add excalidraw -s user -e EXPRESS_SERVER_URL=http://localhost:3000 -- node /path/to/mcp_excalidraw/dist/index.js`
...
| Export scene | `export_scene` | `mcporter call excalidraw.export_scene` | `GET /api/elements` (save to file) |
| Import scene | `import_scene` | `mcporter call excalidraw.import_scene --args '{...}'` | `POST /api/elements/sync` |
| Snapshot | `snapshot_scene` | `mcporter call excalidraw.snapshot_scene --args '{"name":"..."}'` | `POST /api/snapshots` |
| Restore snapshot | `restore_snapshot` | `mcporter call excalidraw.restore_snapshot --args '{"name":"..."}'` | `GET /api/snapshots/:name` then `POST /api/elements/sync` |
| Screenshot | `get_canvas_screenshot` | `mcporter call excalidraw.get_canvas_screenshot` | `POST /api/export/image` (needs browser) |
...
3. **Arrow binding**: MCP accepts `startElementId`/`endElementId`. REST requires `"start": {"id": "..."}` / `"end": {"id": "..."}`.
...
5. **Updating labels via REST**: Re-include `"label"` in the PUT body to ensure it renders correctly after updates.
