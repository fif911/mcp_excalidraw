...
import { measureText } from './utils/textMeasure.js';
import { parseEnhancedD2 } from './utils/enhancedD2Parser.js';
import { buildFromEnhancedD2 } from './utils/enhancedD2Builder.js';
...
// Enhanced D2 → complete diagram build
app.post('/api/elements/from-enhanced-d2', async (req: Request, res: Response) => {
  try {
    const { d2Diagram, export: doExport = true } = req.body;
    if (!d2Diagram || typeof d2Diagram !== 'string') {
      return res.status(400).json({
        success: false,
        error: 'Enhanced D2 diagram definition is required'
      });
    }

    logger.info('Received enhanced D2 build request', { diagramLength: d2Diagram.length });

    const graph = parseEnhancedD2(d2Diagram);
    const result = await buildFromEnhancedD2(graph, { export: doExport });

    broadcast({
      type: 'enhanced_d2_built',
      timestamp: new Date().toISOString()
    } as any);

    res.json(result);
  } catch (error) {
    logger.error('Error processing enhanced D2 diagram:', error);
    res.status(400).json({
      success: false,
      error: (error as Error).message
    });
...
// measureText() imported from utils/textMeasure.ts
