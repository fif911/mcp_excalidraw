...
interface D3Graph {
  shapes: Record<string, D3Shape>;
  connections: D3Connection[];
  arrowStyle: D3ArrowStyle;
}

// ─── D3 Syntax Parser ───────────────────────────────────────────────────

export function parseD3(source: string): D3Graph {
  const shapes: Record<string, D3Shape> = {};
  const connections: D3Connection[] = [];
  const arrowStyle: D3ArrowStyle = {
    strokeColor: '#545B64',
    strokeWidth: 2,
    badgeBg: '#232F3E',
    badgeColor: '#ffffff',
  ...
  const ensureShape = (fullId: string, label?: string): D3Shape => {
    if (!shapes[fullId]) {
...
export function layoutD3Graph(graph: D3Graph): { layout: Record<string, LayoutNode>; changes: LayoutChange[] } {
...
export function convertD3ToExcalidraw(source: string): ConvertResult {
  const graph = parseD3(source);
  const { layout, changes: layoutChanges } = layoutD3Graph(graph);
  const elements: any[] = [];
    ...
    function snapEndpoint(
      ...
      if (isLeaf) {
        ...
        if (isStartPoint) {
          ...
          // End point: always use segment direction for correct entry edge
          dx = pt[0]! - neighbor[0]!;
          dy = pt[1]! - neighbor[1]!;
        }
      } else {
