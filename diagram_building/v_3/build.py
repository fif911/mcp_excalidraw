#!/usr/bin/env python3
"""
Build script for Data Transfer Hub v3 diagram.
Three-phase build: containers -> elements -> arrows+badges

Fixes applied per Critic review:
- DTH UI and User moved outside Customer Account boundary
- Removed phantom arrows (User→DTH, DTH→Cognito)
- Badge 3 reassigned to entry arrow (external → DTH UI)
- Arrow 5 uses container_border_point for SFN endpoint
- Badges 6, 8 use manual label_cx/label_cy
- Managed Account has header icon (file-account)
- Step Functions has no header_bg_color (pink border only)
"""
import sys
import os
import time
...
# Customer's AWS Account (shifted right to create gap for external actors)
container_box("cust-account", 200, 100, 980, 860,
              stroke_color="#545B64", fill_color="transparent",
              icon_file_id="file-cloud-hdr", label_text="Customer's AWS Account",
              label_color="#000000", corner_radius=0)

# AWS Managed Account (with header icon per plan)
...
# DynamoDB — above Authentication box
...
# Get label bottom for CloudFormation (for unlabeled arrow clearance)
...
# Badge on the vertical segment, below DTH (matching reference position)
mid1_x = 350
...
# --- Arrow #3: DTH UI -> Authentication box border (L-shape: left then up) ---
# Ends at Auth box left border (cross-container rule)
# Badge on longest segment (vertical, seg 1) at midpoint, offset RIGHT to clear AWS Cloud border
...
arrow("a3", dth_x, dth_y - R, auth_border_x, auth_border_y,
      waypoints=[(55, dth_y - R), (55, auth_border_y)],
      label_number=3, label_cx=82, label_cy=a3_vert_mid_y,
      **ARROW_CFG)
...
# Badge manually positioned above Authentication box top border (y=230)
...
# NOTE: center_all_badge_texts() no longer needed — numbered_circle()
# now applies vert_correction for persistent centering across reloads.
